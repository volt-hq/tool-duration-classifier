...
export async function main(args) {
	...
	if (!["train", "evaluate", "predict", "benchmark"].includes(action)) throw new Error(`Unknown action: ${action}`);
	const allowed = action === "predict" ? ["--model", "--command", "--input"] : ["--data", "--model"];
	const options = {};
	...
	if (action === "predict") {
		...
		const call = options["--input"] ? JSON.parse(await readFile(options["--input"], "utf8")) :
			{ tool: "bash", arguments: { command: options["--command"] } };
		console.log(JSON.stringify(predict(model, call), null, 2));
		return;
	}
